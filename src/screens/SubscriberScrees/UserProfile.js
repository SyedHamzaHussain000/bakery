...
import {clearProducts, clearToken} from '../../redux/Slices';
...
const UserProfile = ({navigation}) => {
  ...
  return (
   ...
   <PlainHeader
      ...
      />
    <View>
        <View style={{borderColor:'gray',flexDirection:'row',gap:10,marginTop:10,}}>
        <Image  style={{height:responsiveHeight(12),width:responsiveWidth(25),borderRadius:responsiveHeight(8),backgroundColor:'lightgray',borderWidth:1,shadowColor: "#000",
shadowOffset: {
	width: 0,
	height: 2,
},
...
elevation: 3,}} source={{uri:`${baseUrl}user/${user.profilePic}`}}/>
<View style={{justifyContent:'space-around'}}>
  <View>
  <Text style={{color:Color.black,fontSize:responsiveFontSize(2),fontWeight:'bold'}}>{user.userName}</Text>
  <Text style={{color:Color.black,fontSize:responsiveFontSize(1.8)}}>{user.email}</Text>
  </View>
  <Button
